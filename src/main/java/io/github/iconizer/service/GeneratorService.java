...
import com.ifs.iconizer.web.rest.IconizerHelper;
...
public class GeneratorService {
    ...
    public String generateZippedApplication(String applicationId, String applicationConfiguration, String baseName) throws IOException {
        StopWatch watch = new StopWatch();
        watch.start();
        File workingDir = generateApplication(applicationId, applicationConfiguration);
        IconizerHelper.triggerBatch(workingDir.getAbsolutePath(), applicationId, baseName);
    ...
    public String generateTemplateApplication(String applicationId, String applicationConfiguration, String baseName, String applicationTemplateType, String packageName) throws IOException {
        StopWatch watch = new StopWatch();
        watch.start();
        File workingDir = generateApplication(applicationId, applicationConfiguration);
        IconizerHelper.triggerTemplateApplicationBatch(workingDir.getAbsolutePath(), applicationProperties, applicationId, baseName, applicationTemplateType, packageName);
        this.zipResult(workingDir);
        watch.stop();
        log.info("Zipped application generated in {} ms", watch.getTotalTimeMillis());
        return workingDir + ".zip";
